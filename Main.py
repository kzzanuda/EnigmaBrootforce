    ...
    for i in range(len(alphabet)):            # search for rotor 1 start position
        for j in range(len(alphabet)):        # search for rotor 2 start position
            for k in range(len(alphabet)):    # search for rotor 3 start position
                ...
                if (plaintext.find(cribtext) != -1):
                    if plaintext.index(cribtext) == 0:
                        my_file = open("DCTF.txt", "a+")
                        my_file.write(plaintext + '\n')
                        my_file.close()
...
if __name__ == '__main__':
    ...
    ciphertext = []#'OBQWWEOZLMBDNRZHAIOLZTQTJNPPJRXYPLFYDYBQVZNENTSRXHKMRACYBTOVNT'
    with open('cipher_skooby.txt', 'r') as file:
        for string in file:
    ...
    for ciptext in ciphertext:
        print(( "Brute force crypt attack on Enigma message %s using crib %s" % (ciptext, cribtext) ))

        # try all rotor settings (choosing three from five)
        for rotor_setting in rotor:
            print(( "Trying rotors %s..." % (rotor_setting) ))
            rotor_choice, start_pos = find_rotor_start( rotor_setting, ciptext, cribtext)
            if (start_pos != "null"):
